...
label("$48 \;\mathrm{cm}$",shift((0,.05))*P+(L/2,H/2),N, UnFill);

draw(shift((.1,.0))*shift(P)*( (L,H/2)--(L,-H/2) ),brown+dotted+1bp,Arrows(2mm));
label("$32 \;\mathrm{cm}$",shift((.1,.0))*P+(L,0),E, NoFill);

draw(shift((-.5,.0))*( (0,-W/2)--(0,W/2) ),brown+dotted+1bp,Arrows(2mm));
label(rotate(90)*"$224 \;\mathrm{cm}$",(-.55,.0), NoFill);