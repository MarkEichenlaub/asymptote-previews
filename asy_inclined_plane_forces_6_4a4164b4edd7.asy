...
pair o = (.75*L,top.y);
draw(shift(o)*scale(len)*rotate(-theta)*((0,0)--(1,0)), arrow = Arrow(6));
draw(shift(o)*scale(len)*rotate(-theta)*((0,0)--(0,1)), arrow = Arrow(6));
label("$\hat{x}'$",o+rotate(-theta)*(len,0),E);
label("$\hat{y}'$",o+rotate(-theta)*(0,len),N);
