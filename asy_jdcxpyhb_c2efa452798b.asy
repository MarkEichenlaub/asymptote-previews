//this is a template for creating scatter plots of data from physics experiments
  ...
  return 1/(x/4); //change to, e.g. "return x^2;" to plot x^2 on the horizontal axis. 
  }
  ...
  string xAxisLabel = "$V^{-1} \;\;(\mathrm{cm}^{-3})$";
  string yAxisLabel = "$P \;\;(\mathrm{kPa})$";
  ...
  //axes
  real xTickSpacing = 00.04; //must be entered as a decimal
  real xSubtickSpacing = 0.01; //set equal to xTickSpacing if you don't want little ticks
  real yTickSpacing = 40.0;
  real ySubtickSpacing = 10.0;
