...
real[] energies = {(1-5mu)/2, 0, (1+5mu)/2}; //you can make this as many types of energy as you want, but update line 9 as well
...
string title = "$B$ Hits the Ground and Stops";

//----------------- TURN THINGS ON AND OFF --------------

bool show_total = true; //add auto-calculated "total energy" bar at the end

bool show_values_for_energies = false; //writes out the energy above each column
...
label("$\frac{1-5\mu}{2}$",(bar_width+0/num_energies, energies[0]),N );
label("$0$",(bar_width+1/num_energies, energies[1]),N );
label("$\frac{1+5\mu}{2}$",(bar_width+2/num_energies, energies[2]),N );
label("$1$",(bar_width+3/num_energies, energies[3]),N );
