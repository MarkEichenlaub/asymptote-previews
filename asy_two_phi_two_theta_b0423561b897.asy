...
real phi   = 10;      // original angle of incidence
real theta = 20;      // tilt angle of window (visually noticeable)

// --- Geometry ---
real halfSeg   = 2.5;     // half-length of blue window segment
real beamLen   = 6.0;     // length of incident / reflected beams
real normalLen = 3.5;     // half-length of dashed normal line
...
// The incident beam is to the LEFT of the normal.
{
    ...
    label("$\varphi\!+\!\theta$",
          O + (arcR_inc + 0.6) * dir(mid), W, labelPen);
}
...
// The reflected beam is to the RIGHT of the normal.
{
    ...
    real mid = (ab[0] + ab[1]) / 2;
    label("$\varphi\!+\!\theta$",
          O + (arcR_ref + 0.6) * dir(mid), E, labelPen);
}
...
if (showTotalLabel) {
    ...
    label("$2\varphi + 2\theta$",
          O + (arcR_total + 0.55) * dir(mid), S, labelPen);
}