...
draw(shift(rotate(theta)*c)*((0,-g*Cos(theta))--(g*Sin(theta),-g*Cos(theta))), red+dotted);

draw(arc(rotate(theta)*c,.1,-90,-90+theta));
label("$\theta$",rotate(theta)*c + .14*dir(-90+theta/2));
label("$g_x'$",rotate(theta)*c+(Sin(theta)*g/2,-g*Cos(theta))+(.01,0),S,red+dotted);
label("$g_y'$",rotate(theta)*c+(0,-g/2*Cos(theta))+(.01,.03),W,red);