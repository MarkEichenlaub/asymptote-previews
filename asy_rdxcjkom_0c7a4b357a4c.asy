...
//draw(shift(sat)*scale(factor*earth_mass)*(sun--(earth-sat)),arrow = Arrow(6),red);
//label("$a_{\mathrm{Earth}}$",sat+(0.05,0),SE,red);

draw(shift((1-factor)*sat)*scale(factor*earth_mass)*(sun--(earth-sat)),arrow = Arrow(6),red);
label("$a_{\mathrm{Earth}}$",(1-factor)*sat+(0.05,0),SE,red);

draw("$a_c$",com--sat,arrow = Arrow(6),red);
