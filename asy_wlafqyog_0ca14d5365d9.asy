...
real[] ydata = {0.22,0.32,0.40,0.46,0.50,0.55,0.64,0.91}; 
real y_transform(real y){ 
return y; //change to, e.g. "return y^2;" to plot y^2 on the horizontal axis. 
}

real[] xdata = {0.25,0.5,0.75,1.0,1.25,1.5,2.0,4.0}; 
real x_transform(real x){
return x;
}

string xAxisLabel = "Height (m)";
string yAxisLabel = "Time (s)";
...
//axes
real yTickSpacing = 0.1; //must be entered as a decimal
real ySubtickSpacing = 0.05; //set equal to xTickSpacing if you don't want little ticks
real xTickSpacing = 1.0;
