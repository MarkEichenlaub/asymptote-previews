...
label("incident ray",mid+(-4.5,0),N,lightolive);

//angle of incidence
draw(anglemark(norm+mid,mid,(0,mid.y),3),blue);
label(
  "\begin{tabular}{c} angle of \\ incidence \end{tabular}", 
  mid+dir(90+ (90-th)/2)*3*.3, 
  NW, 
  blue
);

//right angle with normal
draw(rightanglemark(norm+mid,mid,(0,0),1.5),dotted);

//angle between ray and mirror
draw(anglemark((0,mid.y),mid,(0,0),4),red);