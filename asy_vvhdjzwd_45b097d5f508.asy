...
draw((-4,0)--(-2.5,0), arrow = Arrow(6));
label("$v_{\rm same}$",(-2.5,0),E);
//draw((-4,-.1)--(-6.5,-.1), arrow = Arrow(6),deepred);
//label("$v$",(-6.5,-.1),W,deepred);
//draw((3,-.1)--(.5,-.1), arrow = Arrow(6), deepred);
//label("$v$",(.5,-.1),SW,deepred);
label("desired reference frame, before collision",(-.5,-1.5),S);
draw((3,0)--(1.5,0), arrow = Arrow(6));
label("$v_{\rm same}$",(1.5,0),W);
