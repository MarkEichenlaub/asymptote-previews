...
real theta(int line){
    ...
    return min_theta + line*d_theta; //angle of line through center of the sphere
}

//the parameter is how many lines the sphere will block; the rest is calculated from that.
//the lines that hit the tangent of the sphere do not count as blocked
picture make_scene(int lines_blocked, int center_line){
	picture p;
    
    
    //finding the center of the sphere, given how many rays it's blocking, which ray its center is on, and its radius
	real fov_blocked = fov*(lines_blocked+1)/(n_lines-1);
    real d = radius/Sin(fov_blocked/2); //distance from light to center of sphere
    pair center = d*dir(theta(center_line));
        
    //sphere
    path sphere = shift(center)*scale(radius)*unitcircle;
    path big_sphere = shift(center)*scale(radius*1.001)*unitcircle; //makes sure tangents intersect
    draw(p,sphere);
    ...
    pair left_tangent = intersectionpoint(big_sphere,left_path);
    pair right_tangent = intersectionpoint(big_sphere, right_path);
    ...
    for(int i = 0; i<n_lines; ++i){     
        ...
        if(abs(i - center_line) <= (lines_blocked-1)/2){
        	pair intersection = intersectionpoint(sphere, ray);
    ...
    filldraw(p,sphere, white);
    
    return p;
}

add(make_scene(3,4));
add(shift(12,0)*make_scene(5,4));
//label("Shadow", shift(-hit_wall(theta(7)).x,-9)*(hit_wall(theta(7)).x, hit_wall(theta(7)).y+.2), N);