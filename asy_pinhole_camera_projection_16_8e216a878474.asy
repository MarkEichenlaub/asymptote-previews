...
pair P2 = (d+1,offset); //pinhole
pair T2 = (1,W/2+offset);//top of wemby
pair B2 = (1,-W/2+offset); //bottom of wemby
pair iT2 = interp(B2,P2,(W+H)/W);
pair iB2 = interp(T2,P2,(W+H)/W);

// draw lower Wemby before clipping so the image exists in the scope
label(reflect((0,0),(0,1))*wemby, (1,.01+offset));
layer();
