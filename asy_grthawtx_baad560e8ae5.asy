...
picture make_scene(int lines_blocked, int center_line){
    ...
    draw( shift(0,.2)*(hit_wall(theta(left_line))--hit_wall(theta(right_line))), arrow=Arrows(TeXHead));
    label("Shadow",(hit_wall(theta(center_line)).x,hit_wall(theta(center_line)).y+.2),N);
