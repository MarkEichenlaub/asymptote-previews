...
real[] xdata = {0.209, 0.234, 0.270, 0.331, 0.466}; 
real x_transform(real x){ 
return x; //change to, e.g. "return x^2;" to plot x^2 on the horizontal axis. 
}

real[] ydata = {1.86, 2.04, 2.40, 3.02, 4.08}; 
real y_transform(real y){
return y;
}

string xAxisLabel = "$\frac{1}{\sqrt{h}} \quad \left(\mathrm{cm}^{-1/2}\right)$";
...
real curve(real x){
return 8.76*x;  //change to the equation for your curve
}
...
real xSubtickSpacing = .02; //set equal to xTickSpacing if you don't want little ticks
