...
pen n_red = rgb(1.0, 0.2, 0.4);
tremble tr = tremble(angle=10, random=3);
path p = tr.deform((1, 1)--(1, 2)--(2, 2)--(2, 1)--cycle);
draw(p, n_red + linewidth(1), Arrow(TeXHead, Relative(0.14)));
add(arrow(TeXHead, p, n_red + linewidth(1), Relative(0.31)));
add(arrow(TeXHead, p, n_red + linewidth(1), Relative(0.6)));
add(arrow(TeXHead, p, n_red + linewidth(1), Relative(0.8)));