...
// -- Beam geometry --
real phi = 7;                 // angle of incidence (degrees from normal)
real beamLen = 5.5;            // length of each beam ray

// -- Normal line --
real normalLen = 5;          // total length of dashed normal (extends above and below)

// -- Angle arcs --
real arcR_phi = 2.3;           // radius for individual phi arcs
real arcR_2phi = 4;          // radius for the combined 2*phi arc
real labelScale = 1.15;        // multiplier to push labels outward from arc
real twoPhiLabelScale = 1.1;  // extra push for the 2*phi label to avoid overlap
...
if (showNormal) {
    ...
    label("normal", mc + (0, normalLen * 0.3), N, fontsize(24) + gray(0.35));
}
